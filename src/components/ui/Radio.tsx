...
export const UiRadio: React.FC<UiRadioOwnProps & ReactAria.RadioProps> = ({
  ...
  <ReactAria.Radio
    ...
    className={twMerge(
      ...
      variant === "outline" &&
        "p-4 gap-4 border border-black/15 hover:border-black/50 group-data-[selected]:border-black transition-colors",
      className as string
    )}
  />
)
...
export const UiRadioBox: React.FC<React.ComponentPropsWithoutRef<"span">> = ({
  ...
  <span
    {...props}
    className={twMerge(
      "border border-black/25 w-4 h-4 block transition-colors rounded-full group-hover:border-black/60 group-data-[selected]:border-black group-data-[selected]:border-6 group-hover:group-data-[selected]:border-black",
      className
    )}
  />
)
