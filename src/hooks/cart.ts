import {
  ...
  deleteLineItem,
  getCartQuantity,
  // getPaymentMethod,   // Stripe disabled
  initiatePaymentSession,
  placeOrder,
  retrieveCart,
  setAddresses,
  setEmail,
  // setPaymentMethod,   // Stripe disabled
  setShippingMethod,
  updateLineItem,
  updateRegion,
} from "@lib/data/cart"
...
// Stripe disabled — stubs keep dead step-components compilable.
// Remove stubs and restore full implementations when Stripe is re-enabled.
...
export const useGetPaymentMethod = (_id: string | undefined) => {
  return useQuery({
    queryKey: ["payment", _id],
    queryFn: async (): Promise<{ card?: { brand?: string; last4?: string } } | null> => null,
  })
}
